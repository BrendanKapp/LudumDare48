    ...
    public class RockStudio : EditorWindow
    {
        ...
        void OnEnable()
        {
            ...
            else
            {
                ...
                basicMenuHeader = new GUIContent("   <b>Basic</b>",
                    EditorGUIUtility.Load("Assets/Ameye/RockStudio/Icons/" + "basic16dark" + ".png") as Texture2D);
                combineMenuHeader = new GUIContent(" <b>Combine</b>",
                    EditorGUIUtility.Load("Assets/Ameye/RockStudio/Icons/" + "combine16dark" + ".png") as Texture2D);
                sculptMenuHeader = new GUIContent("  <b>Sculpt</b>",
                    EditorGUIUtility.Load("Assets/Ameye/RockStudio/Icons/" + "sculpting16dark" + ".png") as Texture2D);
                wearMenuHeader = new GUIContent("  <b> Wear</b>",
                    EditorGUIUtility.Load("Assets/Ameye/RockStudio/Icons/" + "wear16dark" + ".png") as Texture2D);
                settingsMenuHeader = new GUIContent(" <b>Settings</b>",
                    EditorGUIUtility.Load("Assets/Ameye/RockStudio/Icons/" + "settings16dark" + ".png") as Texture2D);
                exportMenuHeader = new GUIContent("  <b>Save</b>",
                    EditorGUIUtility.Load("Assets/Ameye/RockStudio/Icons/" + "save16dark" + ".png") as Texture2D);
            ...
            meshCountIcon = new GUIContent("", EditorGUIUtility.ObjectContent(null, typeof(MeshFilter)).image,
                "The number of meshes that will be combined.");
            ...
            removeSourceMeshIcon = new GUIContent("", EditorGUIUtility.IconContent("d_P4_DeletedLocal").image,
            "Remove source mesh.");
        ...
        void LoadStyles()
        {
            for (int i = 0; i < 4; i++)
            {
                if (rockMaterials[i].mainTexture != null) materialPreviews[i] = rockMaterials[i].mainTexture as Texture2D;
                else if(materialPreviews[i] != null)
                {
        ...
        void OnGUI()
        {
            ...
            switch (toolBarIndex)
            {
                case 0: // Basic Creation
                    ...
                    GUILayout.Label(basicMenuHeader, helpbox,
                    ...
                    switch (typeOfrock)
                    {
                        ...
                        case 1:
                            ...
                            boulderRadius =
                                EditorGUILayout.FloatField(boulderRadius);
                            break;
                        ...
                        case 2:
                            ...
                            baseHeight =
                                EditorGUILayout.FloatField(baseHeight);
                    ...
                    if (GUILayout.Button(GUIContent.none, materialPickers[0], GUILayout.Height(17)))
                    {
                        ...
                        EditorGUIUtility.ShowObjectPicker<Material>(null, true, "",
                            materialPickerIndex);
                    }
                ...
                case 1: // Sculpting
                    CancelPlacement();
                    GUILayout.Label(sculptMenuHeader, helpbox,
                        GUILayout.Height(columnHeight - heightOffset));
                    EditorUtilities.DrawUILine(lineDividerColor, 2, 0);
                    ...
                    {
                        if (GameObject.Find("Sculpture in progress") != null) DestroyImmediate(GameObject.Find("Sculpture in progress"));
                        NewSculpture();
                        AddShape();
                    ...
                    if (GUILayout.Button("Generate"))
                    {
                        if (GameObject.Find("Sculpture in progress") != null)
                        {
                            ...
                            if (modeOfComposition == 1)
                                SculptureCreator.GenerateComposition(numberOfVerticesInt, Vector3.zero, Vector3.zero,
                                    rockMaterial);
                            else
                                SculptureCreator.GenerateCompositionFill(numberOfVerticesInt, Vector3.zero, Vector3.zero,
                                    rockMaterial);
                    ...
                    if (GUILayout.Button("Save"))
                    {
                        if (GameObject.Find("Sculpture") != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (colliderType == 0) newStructure.AddComponent(typeof(BoxCollider));
                                if (colliderType == 1)
                                {
        ...
        void drawGizmo()
        {
            ...
            if (currentlyPlacingRock)
            {
                if (mouseHitPoint.transform)
                {
                    ...
                    if (placementToolLocation == null) placementToolLocation = new GameObject("Gizmo Location").transform;
                    placementToolLocation.rotation = mouseHitPoint.transform.rotation;
                    placementToolLocation.forward = mouseHitPoint.normal;

                    Handles.ArrowHandleCap(3, mouseHitPoint.point, placementToolLocation.rotation,
                        1 * placementToolSize, EventType.Repaint);

                    Handles.CircleHandleCap(2, currentMousePosition, placementToolLocation.rotation, placementToolSize,
                        EventType.Repaint);
                    Handles.color = innerColor;
                    Handles.DrawSolidDisc(currentMousePosition, mouseHitPoint.normal, placementToolSize);

                    placementToolLocation.up = mouseHitPoint.normal;
                }
            }
            else
            {
                if (placementToolLocation) DestroyImmediate(placementToolLocation.gameObject);
            }
        }
        ...
        void sceneInput()
        {
            ...
            {
                if (currentEvent.delta.y > 0 && placementToolSize < 3f) placementToolSize = placementToolSize + 0.1f;

                else if (currentEvent.delta.y < 0 && placementToolSize > 0.1f)
                {
                    placementToolSize = placementToolSize - 0.1f;
                }
                Repaint();
            }
        }
        ...
        {
            if (Event.current.type == type && Event.current.modifiers == codeModifier &&
                Event.current.keyCode == hotkey)
            {
                Event.current.Use();
                return true;
            }
        ...
        void updateMousePos(SceneView sceneView)
        {
            ...
            RaycastHit hit;
            Ray ray = sceneView.camera.ScreenPointToRay(new Vector2(currentEvent.mousePosition.x,
        ...
        private void GenerateRock(Vector3 pos, Vector3 nor)
        {
            ...
            switch (typeOfrock)
            {
                case 0:
                    points =
                        MeshUtilities.GetRandomPointsWithinCube(numberOfVerticesInt, edgeWidth, edgeHeight, edgeDepth);
                    break;
                case 1:
                    points = MeshUtilities.GetRandomPointsWithinSphere(numberOfVerticesInt, boulderRadius);
                    break;
                case 2:
                    points = MeshUtilities.GetRandomPointsWithinCrystal(numberOfVerticesInt, tetragonal, oneSided,
                        baseWidth, baseHeight, tipProtrusion, tipFlatness);
                    break;
            ...
            if (GameObject.Find(rockGroupName) == null)
            {
            ...
            else rockGroup = GameObject.Find(rockGroupName);

            RockCreation.CreateRock(points, pos + new Vector3(0, rockYPositionOffset, 0), nor, rockOrientation,
                rockMaterial, rockName, generateWithRigidbody, colliderType, rockGroup, showGenerationInfo);
        }
    }
